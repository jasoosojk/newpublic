...
bot = telebot.TeleBot('8017156147:AAE9y5Wl8Qvn6jHV7Obwgursnp_Sb42bpEQ')
...
def clear_users_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "ꜰʀᴇᴇ ᴋᴇ ᴅʜᴀʀᴍ ꜱʜᴀʟᴀ ʜᴀɪ ᴋʏᴀ ᴊᴏ ᴍᴜ ᴜᴛᴛʜᴀ ᴋᴀɪ ᴋʜɪ ʙʜɪ ɢᴜꜱ ʀʜᴀɪ ʜᴏ ʙᴜʏ ᴋʀᴏ ꜰʀᴇᴇ ᴍᴀɪ ᴋᴜᴄʜ ɴʜɪ ᴍɪʟᴛᴀ ʙᴜʏ:- @SIDIKI_MUSTAFA_47 🙇."
    bot.reply_to(message, response)
...
def show_recent_logs(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝙏𝙝𝙞𝙨 𝘽𝙤𝙩 𝙞𝙨 𝙤𝙣𝙡𝙮 𝙛𝙤𝙧 𝙥𝙖𝙞𝙙 𝙪𝙨𝙚𝙧𝙨 𝙗𝙪𝙮 𝙣𝙤𝙬 𝙛𝙧𝙤𝙢 - @SIDIKI_MUSTAFA_47 \n205 KALA JADU "
        bot.reply_to(message, response)
...
def welcome_start(message):
    ...
    response = f'''🌟  DDOS 🌟\n🔥 OWNER -> @SIDIKI_MUSTAFA_47\n🔥 CHANNEL -> WAIT WHAT 🧐 -> /mustafa 🌟'''
    bot.reply_to(message, response)
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "𝘼𝙇𝙀𝙍𝙏 ⚠️‼️\n𝙏𝙃𝙄𝙎 𝙈𝙀𝙎𝙎𝘼𝙂𝙀 𝙎𝙀𝙉𝙏 𝙁𝙍𝙊𝙈 :-->https://t.me/+7RW5UStfEDUwZDE1 ✅:\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
